  ...
  return (
    ...
    >
      ...
      <header className="flex-none flex flex-col items-center pt-12 pb-6 px-4">
        {isVictory ? (
          <>
            <div className="font-pixel text-r-12 text-yellow-400/80 tracking-widest mb-2">
              ✦ GAUNTLET CLEARED ✦
            </div>
            <h1
              className="font-pixel text-r-27 tracking-wider"
              style={{
                color: '#fbbf24',
          ...
          <>
            <div className="font-pixel text-r-12 text-red-700/70 tracking-widest mb-2">
              ✦ RUN ENDED ✦
            </div>
            <h1
              className="font-pixel text-r-27 tracking-wider"
            ...
            <p className="mt-3 font-pixel text-r-9 text-red-500/50 tracking-wider text-center">
      ...
      {!isVictory && (
        <section className="flex-none px-4 pb-5">
          <div className="font-pixel text-r-8 text-red-800/50 tracking-widest text-center mb-3">
            — GAUNTLET PROGRESS —
          </div>
      ...
      <section className="flex-none px-4 pb-6">
        <div className="font-pixel text-r-9 text-red-700/60 tracking-widest mb-3 text-center">
          — LAST CREW STANDING —
        </div>
      ...
      <footer className="flex-none px-4 pb-10">
        ...
        <button
          ...
          className="
            w-full py-4 rounded
            font-pixel text-r-15 tracking-widest
        ...
        <p className="mt-3 text-center font-pixel text-r-8 text-red-900/60 tracking-wider">
...
const StatRow: React.FC<{
  ...
  <div className="flex items-center justify-between px-4 py-3 border-b border-red-900/30 last:border-b-0">
    <span className="font-pixel text-r-9 text-red-700/70 tracking-wider">
      {label}
      {isPersonalBest && (
...
const EndCrewSlot: React.FC<{ index: number; crewId: number | null }> = ({ index, crewId }) => {
  ...
  return (
    ...
    >
      {/* Slot number */}
      <div className="font-pixel text-r-8 text-red-800/60">{index}</div>
      ...
      >
        {crewId ? (
          <span className="text-r-24 leading-none">
            {CREW_EMOJI[crewId] ?? (name ?? '?').charAt(0)}
          </span>
        ) : (
          <span className="font-pixel text-r-11 text-red-900/40">—</span>
        )}
      </div>

      {/* Name */}
      <div className={[
        'font-pixel text-r-6 text-center leading-tight',
