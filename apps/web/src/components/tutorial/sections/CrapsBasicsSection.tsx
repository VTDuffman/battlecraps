...
const ReferenceCard: React.FC<ReferenceCardProps> = ({ title, icon, children }) => (
  <div className="border border-white/10 rounded bg-white/[0.03] p-4 space-y-2">
    <div className="flex items-center gap-2">
      ...
      <span className="font-pixel text-r-8 text-amber-300 tracking-widest">{title}</span>
    </div>
    <div className="font-mono text-r-9 text-white/70 leading-relaxed space-y-1">
      {children}
    </div>
  </div>
);
...
const Outcome: React.FC<{ label: string; color: string; children: React.ReactNode }> = ({
  ...
  <div className="flex items-start gap-2">
    <span className={`font-pixel text-r-7 ${color} flex-none pt-0.5`}>{label}</span>
    <span>{children}</span>
  </div>
);
...
export const CrapsBasicsSection: React.FC = () => (
  <div className="space-y-3 pb-4">
    ...
    <ReferenceCard title="COME-OUT ROLL" icon="🎲">
      <p className="text-white/50 text-r-8 mb-2">
    ...
    <ReferenceCard title="PASS LINE" icon="🎯">
      ...
      <p className="text-white/40 text-r-8 mt-1">
      ...
      <div className="grid grid-cols-3 gap-1 mt-1 text-center">
        {[
          ...
          <div key={point} className="border border-white/10 rounded p-1.5 bg-black/20">
            <div className="text-amber-300 text-r-8 font-pixel">{point}</div>
            <div className="text-green-400 text-r-8 font-pixel mt-0.5">{pays}</div>
            <div className="text-white/30 text-r-7 mt-0.5">max {max}</div>
          </div>
        ))}
      </div>
      <p className="text-white/40 text-r-8 mt-1">
