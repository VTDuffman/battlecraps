...
## Highest KI number assigned: KI-073

Last assigned: KI-073 — Member's Jacket comp: pip display fires one floor early; shooter bonus lost after Floor 3 (2026-05-21)
...
- `apps/api/src/routes/recruit.ts` — comp perk ID persistence; EXTRA_SHOOTER gate silently swallowed all other comp types (KI-063); shooter bonus display possibly wrong (KI-069); comp write targets wrong table — user row instead of run row (KI-072); compShooterBonus not reading run.compPerkIds so bonus resets on every non-boss pub (KI-073)
- `apps/web/src/components/TableBoard.tsx` — pip-capacity heuristic uses wrong marker-index threshold; should be derived from shooters value (KI-073)
...
- **Meta-progression persistence gap** — feature branch (KI-051) fixed enforcement of a perk that was never being persisted; root cause (KI-063) found later during playtesting; KI-072 reveals a third layer: comp perk IDs written to the wrong DB table (user row vs. run row), causing cross-run bleed; KI-073 reveals a fourth layer: even when perk IDs are correctly written to run.compPerkIds, the compShooterBonus derivation in recruit.ts never reads them — only checking the immediately preceding boss marker. Pattern: comp-system bugs can exist at the DB write layer, the enforcement read layer, the table-scoping layer, or the within-handler re-read layer — verify all four before closing any comp-related issue.
- **UI-vs-server state derivation mismatches** — client-side display conditions computed from marker-index heuristics rather than server-authoritative state (shooters value); KI-073 is the canonical example. Prefer reading game-state values directly rather than re-deriving them from marker index.
...
## Active Testing Focus (2026-05-21)

Playtester observation logged: Member's Jacket comp bugs (pip display + shooter bonus persistence) — KI-073.
