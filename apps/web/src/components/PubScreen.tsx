  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1 flex-wrap">
        <div className={['self-start px-1.5 py-0.5 rounded text-r-8 font-pixel', cat.bg, cat.text].join(' ')}>
          {cat.label}
        </div>
        <div className={['self-start px-1.5 py-0.5 rounded text-r-8 font-pixel', rarity.bg, rarity.text].join(' ')}>
          {crew.rarity.toUpperCase()}
        </div>
      </div>

      {/* Name */}
      <div className="font-pixel text-r-10 text-amber-100 leading-relaxed">
        {crew.name}
      </div>

      {/* Description */}
      <div className="font-mono text-r-10 text-amber-300/60 leading-tight flex-1">
...
const SlotButton: React.FC<SlotButtonProps> = ({ index, occupantId, isSelected, onClick }) => (
  ...
  >
    {occupantId ? (
      <span className="text-r-28 leading-none">{CREW_EMOJI[occupantId] ?? '?'}</span>
    ) : (
      <>
        <div
          className={[
            'w-5 h-5 rounded flex items-center justify-center font-pixel text-r-11',
            isSelected ? 'bg-amber-500 text-stone-900' : 'bg-stone-700 text-amber-300/60',
          ].join(' ')}
        >
          {index}
        </div>
        <div className="font-pixel text-r-10 text-center leading-tight text-stone-600">
          EMPTY
        </div>
      </>
    )}
...
const PubFireSlot: React.FC<PubFireSlotProps> = ({ crewId, crewName, description, onFire }) => {
  ...
  if (!crewId) {
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-1 px-2 rounded border border-stone-700/20 bg-stone-900/20 min-w-0" style={{ height: 'clamp(40px,5.5dvh,64px)' }}>
        ...
        <div className="font-pixel text-r-10 text-stone-700">EMPTY</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {/* Emoji portrait */}
      <div className="text-r-33 leading-none">{CREW_EMOJI[crewId]}</div>
      ...
      {description && (
        <div className="
          ...
          font-mono text-r-11 text-white/90 leading-snug
      ...
      {onFire && (
        <button
          ...
          className={[
            ...
            'font-pixel text-r-11 leading-none',
  ...
  return (
    ...
    >
      ...
      <header className="relative flex-none px-4 text-center" style={{ paddingTop: 'clamp(8px,1.2dvh,32px)', paddingBottom: 'clamp(6px,0.9dvh,24px)' }}>
        ...
        <div className="font-pixel text-r-11 tracking-widest mb-1" style={{ color: theme.pubSubtextColor }}>
          ✦ MARKER CLEARED ✦
        </div>
        <h1
          className="font-pixel text-r-21 tracking-wide"
          style={{ color: theme.pubTitleColor, textShadow: theme.pubTitleShadow }}
        >
          {theme.pubName}
        </h1>
        <div className="mt-1 font-mono text-r-18" style={{ color: theme.pubSubtextColor }}>
        ...
        <div className="mt-2 flex justify-center gap-4">
          <div className="text-center">
            <div className="font-pixel text-r-11 mb-0.5" style={{ color: theme.pubSubtextColor }}>BANKROLL</div>
            <div className="font-pixel text-r-16" style={{ color: theme.accentBright }}>{bankrollDisplay}</div>
          </div>
          <div className="text-center">
            <div className="font-pixel text-r-11 mb-0.5" style={{ color: theme.pubSubtextColor }}>SHOOTERS</div>
            <div className="font-pixel text-r-16" style={{ color: theme.accentBright }}>
              {upcomingShooters} {'✦'.repeat(5)}
              {isComped && <span className="ml-px" style={{ color: theme.accentBright }}>✦</span>}
            </div>
            {isComped && (
              <div className="font-pixel text-r-10 tracking-widest mt-0.5" style={{ color: theme.accentBright }}>
                +1 COMP
              </div>
            )}
          </div>
      ...
      <section className="relative flex-none px-3" style={{ paddingBottom: 'clamp(4px,0.6dvh,16px)' }}>
        ...
        {/* Loading state */}
        {draftLoading && (
          <div className="text-center font-mono text-r-18 text-amber-300/40 py-4 animate-pulse">
            Loading crew…
          </div>
        )}
        ...
        {!draftLoading && (
          <div className={['grid gap-1', pubDraft.length === 2 ? 'grid-cols-2 w-2/3 mx-auto' : 'grid-cols-3'].join(' ')}>
            ...
            {pubDraft.length === 0 && (
              <div className="col-span-3 text-center font-mono text-r-18 text-amber-300/40 py-4">
      ...
      {selectedCrew && (
        <section className="relative flex-none px-3" style={{ paddingBottom: 'clamp(4px,0.6dvh,16px)' }}>
          ...
          >
            <div className="font-pixel text-r-11 text-amber-400/70 mb-1 text-center">
              PLACE&nbsp;
            ...
            {selectedSlot !== null && crewSlots[selectedSlot]?.crewId != null && (
              <div className="mt-2 text-center font-mono text-r-16 text-amber-500/70">
            ...
            <button
              ...
              className={[
                'mt-2 w-full py-1.5 rounded font-pixel text-r-11 tracking-wider',
                'border transition-all duration-150',
                canHire
            ...
            <button
              ...
              className="mt-1 w-full py-0.5 font-pixel text-r-10 text-amber-600/50 hover:text-amber-400/70 transition-colors"
            >
              CANCEL
            </button>
          </div>
      ...
      {errorMsg && (
        <div className="px-4 py-2 mx-3 mb-2 rounded border border-red-700/50 bg-red-900/20">
          <p className="font-mono text-r-18 text-red-300 text-center">{errorMsg}</p>
        </div>
      )}
